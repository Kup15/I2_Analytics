...
import { TAG_TYPES, TAG_TYPE_LABELS } from '@/lib/games-types';
...
const GameTaggingPage = () => {
  ...
  const handleTag = async (typeValue: string, scoreOverride?: number) => {
    ...
    const def = TAG_TYPES.find(t => t.value === typeValue);
    if (!def) return;
      ...
      <div className="mx-auto max-w-3xl">
        ...
        {isCoach && !isClosed && (
          <div className="mb-4">
            <div className="grid grid-cols-3 gap-2 sm:grid-cols-5">
              {TAG_TYPES.map(t => (
                <button
                  ...
                  className={`rounded-xl ${t.color} bg-opacity-90 px-2 py-3 text-white text-sm font-bold active:scale-95 transition-transform`}
                >
                  <div>{t.label}</div>
                  <div className="text-[10px] opacity-80">{t.defaultScore > 0 ? `+${t.defaultScore}` : t.defaultScore}</div>
                </button>
              ))}
            </div>
            {pendingType && (
              <div className="mt-2 flex items-center gap-2 rounded-lg bg-secondary p-2">
                <span className="text-xs text-muted-foreground">ציון מותאם ל"{TAG_TYPE_LABELS[pendingType]}":</span>
                {[-2, -1, 0, 1, 2].map(s => (
            ...
            {tags.length > 0 && (
              <Button variant="outline" size="sm" onClick={handleUndo} className="mt-2 w-full">
                בטל תיוג אחרון
              </Button>
            )}
          </div>
        ...
        <div className="rounded-xl bg-card p-3">
          ...
          {sortedTags.length === 0 ? (
            ...
            <ul className="divide-y divide-border">
              {sortedTags.map(tag => {
                const def = TAG_TYPES.find(t => t.value === tag.type);
                ...
                return (
                  <li key={tag.id} className="flex items-center justify-between gap-2 py-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`inline-block w-2 h-2 rounded-full ${def?.color ?? 'bg-muted'}`} />
                    ...
                    >
                      <div className="text-sm font-medium">{TAG_TYPE_LABELS[tag.type] ?? tag.type}</div>
