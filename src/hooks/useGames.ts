...
// Until `supabase gen types` is rerun against the new schema, the generated
// Database type doesn't know about `games` / `game_tags`. Cast at the boundary.
...
export function useGame(gameId: string | undefined) {
  ...
  const fetch = useCallback(async () => {
    ...
    const [{ data: g }, { data: t }] = await Promise.all([
      ...
      sb.from('game_tags').select('*').eq('game_id', gameId).order('created_at', { ascending: true }),
    ]);
...
export async function addTag(gameId: string, tag: GameTagInput): Promise<{ error?: string }> {
  const { error } = await sb.from('game_tags').insert({ game_id: gameId, ...tag });
  return error ? { error: error.message } : {};
}

export async function deleteTag(tagId: string): Promise<{ error?: string }> {
  const { error } = await sb.from('game_tags').delete().eq('id', tagId);
  return error ? { error: error.message } : {};
}

export async function closeGame(gameId: string): Promise<{ error?: string }> {
  const { error } = await sb.from('games').update({ status: 'closed' }).eq('id', gameId);
  return error ? { error: error.message } : {};
}
